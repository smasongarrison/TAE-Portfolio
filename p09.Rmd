...
```{r read-data}
if(file.exists("data/fix_report.txt")){
...
# Remove the prefix t_ from the beginning of column names.
  ...
  set.seed(9009)
source("p09/helper_functions.R")
# Values taken from the knitted p09 output.
n_rows_target <- 65876
n_trial_units <- 15196
n_participants <- 32
n_blocks <- 15
...
One thing to note here, is that the number of trials differed by condition, with more absent trials than singleton-present trials. 
...
Here, we are using accuracy as a basic check of performance. If accuracy is very low in one condition, response time patterns could be harder to interpret.
...
Here's what I (Mason) was thinking for the heat maps. 

```{r data-for-heatmap}
bins <- 45
fake_heatmap_data <- fix_raw %>%
  filter(
    phase == "experiment",
    !is.na(current_fix_x),
    !is.na(current_fix_y)
  ) %>% 
  mutate(current_fix_x_bin = as.numeric(cut(current_fix_x, bins)),
         current_fix_y_bin = as.numeric(cut(current_fix_y, bins))) 

fake_heatmap_data <- fake_heatmap_data %>% 
#  group_by(current_fix_x_bin, current_fix_y_bin) %>%
  summarise(.by = c(current_fix_x_bin, current_fix_y_bin),
current_fix_duration_binned = mean(current_fix_duration, na.rm = TRUE)) %>%
  ungroup() %>%
  right_join(fake_heatmap_data, by = c("current_fix_x_bin", "current_fix_y_bin"))

item_locations <- item_locations %>%
  mutate(item_x_bin = as.numeric(cut(item_x, bins)),
         item_y_bin = as.numeric(cut(item_y, bins)))
```

```{r heatmap, fig.width=8, fig.height=5*16}

core_plot <- ggplot(fake_heatmap_data, 
                    aes(x = current_fix_x, 
                        y = current_fix_y, group = subjnum
                        )) 

binned_plot <- core_plot +
  geom_bin2d(binwidth = c(45, 45)
             )+# bin size of 45 pixels
  scale_x_continuous(limits = c(0, screen_width), expand = c(0, 0)) +
  scale_y_reverse(limits = c(screen_height, 0), expand = c(0, 0)) + 
  geom_point(
    data = item_locations,
    aes(x = item_x, y = item_y),
    inherit.aes = FALSE,
    shape = 21,
    fill = "white",
    color = "black",
  ...
  geom_text(
    data = item_locations,
    aes(x = item_x, y = item_y, label = item_loc),
    inherit.aes = FALSE,
    color = "black",
    size = 3
  )

# adds a summary statistic (mean fixation duration) to each bin, which is then visualized using color intensity. The `stat = 'identity'` argument tells `geom_bin2d` to use the provided summary statistic directly for the fill color of each bin, rather than counting the number of observations in each bin.
duration_plot <- core_plot +
  geom_bin2d(stat = 'identity',
             aes(fill = current_fix_duration_binned,
                 x = current_fix_x_bin,
                 y = current_fix_y_bin
                 ))+
  scale_x_continuous(breaks = c(1,bins), labels = c(0, screen_width))+
  scale_y_reverse(breaks = c(1,bins), labels = c(screen_height,0)) + 
  geom_point(
    data = item_locations,
    aes(x = item_x_bin, y = item_y_bin),
    inherit.aes = FALSE,
    shape = 21,
    fill = "white",
    color = "black",
  ...
  geom_text(
    data = item_locations,
    aes(x = item_x_bin, y = item_y_bin, label = item_loc),
    inherit.aes = FALSE,
    color = "black",
    size = 3
  )
...
duration_plot +
  scale_fill_viridis_c(
    option = "magma",
    trans = "sqrt",
    name = "Simulated fixation Duration (ms)"
  )  +
  coord_fixed() +
  labs(
    title = "Simulated Fixation Heatmap",
    subtitle = "Fake fixation coordinates calibrated to the structure and summaries shown in the knitted p09 output",
  ...
  #  legend.position = "bottom",
    # text should be vertical in the legend
  #  legend.text = element_text(angle = 270, vjust = 0.5)
  ) #+
 # facet_wrap(~subjnum, ncol = 4)

binned_plot  +   scale_fill_viridis_c(
    option = "viridis",
    trans = "sqrt",
    name = "Simulated fixation Frequency"
  )  +
  coord_fixed() +
  labs(
    title = "Simulated Fixation Heatmap",
